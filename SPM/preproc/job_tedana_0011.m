function [ job ] = job_tedana_0011( meinfo, prefix, outdir, mask, par )
%JOB_TEDANA for tedana version job_tedana_0011
%
...
for iJob = 1 : nJobs
    ...
    % Use old naming convention (no need for BIDS)
    cmd =                              sprintf('%s --convention orig \\\\\n', cmd                );
    ...
    for iSer = 1 : length(series)
        ...
        if par.run % use the normal method
            ...
            if ~isempty(outdir) && ischar(outdir)
                ...
                ser.addVolume(['^' outdir '$'], [    '^s0vG' ext '$'],     's0vG', 1 );
                ser.addVolume(['^' outdir '$'], [   '^t2svG' ext '$'],    't2svG', 1 );
        ...
        elseif par.sge % add the new volume in the object manually, because the file is not created yet
            ...
            if ~isempty(outdir) && ischar(outdir)
                ser.addVolume( 'root', fullfile(ser.path,outdir,[   'ts_OC' ext]),    'ts_OC' );
                ser.addVolume( 'root', fullfile(ser.path,outdir,['dn_ts_OC' ext]), 'dn_ts_OC' );
                ser.addVolume( 'root', fullfile(ser.path,outdir,[     's0v' ext]),      's0v' );
                ser.addVolume( 'root', fullfile(ser.path,outdir,[    't2sv' ext]),     't2sv' );
            elseif ~isempty(outdir) && iscellstr(outdir)
                error('not coded yet')
            else
