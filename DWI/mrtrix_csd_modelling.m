...
% MRTRIX_CSD_MODELLING : CSD voxel Modelling using Mrtrix command dwi2fod
%
%
% DESCRIPTION :
%     This function estimates ODFs using Constrained Spherical Deconvolution (CSD).
%     It is configured via parameters (see below) to support either tractography or Fixel-Based Analysis (FBA) steps, 
%     By default, it computes only the FODs required for tractography.
%     It allows a local execution or a generation of cluster jobs.
%
% WORKFLOW (Available Features) :
%     * Compute average fibre response function (f-response).
%     * Upsample DWI file and mask.
%     * Compute FODs (default).
%     * Intensity normalisation.
%
%     By default, this function only computes the ODFs.
%
% INPUTS : 
%
%     fdwi : cellstr, paths to the preprocessed Diffusion-Weighted Imaging (DWI) files.
%     par  : matvol and MRtrix3 parameters structure.
%              
%             .do_responsemean            : (1 or 0) Enable/disable average fibre response computation.
%             .csd_type                   : (char) Algorithm chosen for ODF computation (e.g., csd, msmt_csd, 'ss3t_csd_beta1', etc.).
%             .do_intensity_normalisation : (1 or 0) Enable/disable final intensity normalisation.
%              ... (other specific 'dwi2fod' and 'matvol' parameters)
% 
% OUTPUT:
%
%     The output FOD image(s) and responsef/mask files if generated, 
%     or the job files required for running the process on a cluster.
%
% REQUIREMENTS:
%     module load FreeSurfer/xxx 
%     Replace 'xxx' with the required available version number.
%
% NOTE    If the chosen CSD algorithm is ss3t_csd_beta1 (Single-Shell 3-Tissue CSD), 
%     ensure that the MRtrix3 installation.
% 
% --------------------------------------------------------------------------
...
defpar.response_gm  = 'response_wm.txt';   % (char) 'response_gm.txt' GM RF filename. File must be in the same dir as fdwi input
defpar.response_csf = 'response_wm.txt';   % (char) 'response_csf.txt' CSF RF filename. File must be in the same dir as fdwi input.
