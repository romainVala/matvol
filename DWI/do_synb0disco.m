...
% export PATH="/network/iss/apps/tools/singularity/***/bin:$PATH"
%
% If running the jobs in cluster use (or look for other versions) :
...
defpar.img      = {'/network/iss/cenir/software/irm/conda_env/singularity/synb0-disco_v3.1.sif'};   % path to singularity synb0-disco image container
...
for nbr =1:length(dwork)
    ...
    if ~par.notopup
        notopup = '';
         ...
         fjson = cellstr(char(gfile(pdirb0(nbr),par.json_reg)));  % If fjson empty ?
        
        try
                   
            acqp  = topup_param_from_json_cenir([fb0(nbr) ; fb0(nbr)],[], [fjson(1) ; fjson(1)]);  % combine files to use the function
        catch
