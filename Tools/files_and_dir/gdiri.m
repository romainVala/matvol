function [output, not_found] = gdiri(indir, reg_ex, varargin)
...
if ~isempty(varargin)
    ...
    output = gdiri(indir, reg_ex); % do the first arguments before
    for nArgIn = 1:length(varargin)
        output = gdiri(output, varargin{nArgIn}); % Then do the other arguments
    end
    ...
    for d = 1:length(dir_content)
        ...
        for nb_reg = 1:length(reg_ex)
            ...
            if strcmp(reg_ex{nb_reg}(1),'-')
                if dir_content(d).isdir && ~isempty(regexpi(dir_content(d).name,reg_ex{nb_reg}(2:end), 'once'))
                    break % to avoid that 2 reg_ex adds the same dir
                end
            end
            
            if dir_content(d).isdir && ~isempty(regexpi(dir_content(d).name,reg_ex{nb_reg}, 'once'))
