...
%
% Inputs 
%        fqsub    : cellstr path to file do_qsub
...
cm = '--depend=afterok:$jobid';
if contains(arguments_lastGroup,'--depend=afterok:$jobid')
    cm = '';
end

arguments_lastGroup{newind(end)} = sprintf('--array=%d-%d %s',nbr + 1, grps(2),cm);
